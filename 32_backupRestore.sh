...
make_backup () {
    date=`date +%Y%m%d`
    echo "Listado de las bases de datos"
    sudo -u postgres psql -c "\l"
    read -p "Elegir la base de datos a respaldar: " dbBackup
    echo -e "\n"

    #verificamos que existe el directorio
    if [ -d "$1" ]; then
        echo "Establecer permisos del directorio"
        #Con -S podemos agregar un comando "chmod" luego de sudo
        echo "$passeword" | sudo -S chmod 755 $1
        echo "Realizando el respaldo..."
        sudo -u postgres pg_dump -Fc $dbBackup > "$1/dbBackup$date.bak"
        echo "Respaldo realizado correctamente en $1/dbBackup$date.bak"
    else
...
restore_backup () {
    ...
    read -p "Ingresar el directorio del respaldo" backupDir
    ls -la $backupDir
    read -p "Elegir respaldo a restaurar" backupToRestore
    echo -e "\n"
    read -p "Ingrese el nombre de la base de datos de destino" destinyDB
    
    #Verificamos si existe la base de datos
    exist=$(sudo -u postgres psql -lqt | cut-d \| -f 1 | grep -wq $destinyDB)
    if [ $exist -eq 0 ];then
        echo "Restaurando en la BDD de destino: destinyDB"
        
    else
        sudo -u postgresql psql -c "create database $destinyDB"
    fi


    if [ -f "backupToRestore" ];then

        echo "Restaurando el respaldo"
        sudo -u postgres pg_restore -Fc -d $destinyBD "$backupDir/$backupToRestore"

        echo "Listado de las bases de datos"
        sudo -u postgres psql -c "\l"
    else
        echo "El respaldo $backupToRestore no existe"
